<template>
  <div>
    <!-- จัดการผลออกรางวัล หวยหุ้นไทย -->
    <div
        id="modal-manage-result-lottery-share-thai"
        class="modal"
        tabindex="-1"
        aria-hidden="true"
      ...
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
                ...
                <div class="flex">
                  <div>
                    <input 
                      ...
                      class="form-control w-auto mr-2" @input="checkValidateFirstPrize()">
          ...
          <div class="modal-footer">
            <button class="btn bg-green-500 text-white mr-1" @click="getResultLottoryShareThai()">ดึงข้อมูลหวย</button>
            <button class="btn btn-secondary w-24 mr-1 mb-2" data-dismiss="modal">
              ปิด
            </button>
            <button class="btn bg-blue-500 text-white w-24 mr-1 mb-2" @click="submitManageResultLotteryShareThai()">
              บันทึก
            </button>
          </div>
        </div>
...
export default {
    props : ['setFormResultShareThai','resultLottoShareThai','statusLoading'],
    data() {
        return {
            form : {
                first_prize : ''
    ...
    watch : {
        setFormResultShareThai : 'setFormShareThai',
        resultLottoShareThai : 'setResultLottoShareThai'
    },
    methods : {
        checkValidateFirstPrize() {
        ...
        submitManageResultLotteryShareThai() {
            if (
                this.$v.form.first_prize.$invalid == false && this.validateFirstPrize == false
            ) {
                this.$emit('confirmManageResultLottoShareThai',this.form);
            } else {
        ...
        getResultLottoryShareThai() {
            this.$emit('getResultLottoryShareThai');
        },
        setResultLottoShareThai(data) {
            // console.log(data);
