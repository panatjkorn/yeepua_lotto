    ...
    <div>
        <div class="box overflow-x-auto mt-5">
            <div class="grid grid-flow-col grid-cols-2 gap-4 mt-5 py-4 px-4">
                ...
                <input v-model="search" type="text" placeholder="ค้นหา..." class="form-control mr-2" @keyup.enter="filterBillCancelType()">
                <button @click="filterBillCancelType()" class="btn bg-yellow-500 text-white mr-2">ค้นหา</button>
                </div>
            </div>
            <div class="col-span-12 lg:col-span-4 text-left px-4 overflow-x-auto">
                <table class="table mt-5">
                    <thead>
                        <tr>
                        <th v-for="(col,index) in tbShowBillCancel.columns" :key="index" class="border-b-2 dark:border-dark-5 whitespace-nowrap">
                            <b>{{ col.label }}</b>
                        </th>
                        </tr>
                    </thead>
                    <tbody v-if="tbShowBillCancel.rows.length > 0">
                        <tr v-for="(data,index) in tbShowBillCancel.rows" :key="index">
                            <td>{{ data.num }}</td>
                            ...
                            <td>
                                ...
                                <p v-for="(bankAccount,index) in data.account_name" :key="index">{{ bankAccount.name }}</p>
                                </div>
                            </td>
...
export default {
    ...
    watch : {
        dataShowBillCancel : 'setDataBillCancel',
        filter_type : 'clearDataFilterSearch'
    },
    methods: {
        setDataBillCancel(data) {
            this.tbShowBillCancel.rows = data.bills.map((item,index) => {
                // console.log(item);
                return {
                    num : index+1,
                    datetime_buy : item ? moment(item.created_at).format("YYYY-MM-DD HH:mm") : '',
                    bill_code : item ? item.bill_code : '',
        ...
        clearDataFilterSearch() {
            this.search = '';
        },
        filterBillCancelType() {
            const _data = {
                type_id : this.filter_type,
                search : this.search
            }
            this.$emit('filterBillCancelType',_data);
        }
    }
}
</script>